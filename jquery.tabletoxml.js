jQuery.fn.tabletoxml = function (options) {
    ...
    $(this).each(function () {
        ...
        var loop = 0;
        var head = 0;
        if(settings.rootnode == "TraitementEtSalaire"){
        var xml = "<" + settings.rootnode + " xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">";
        }else
        ...
        $(this).find('tr').each(function () {
            if (head == 1 ) {
                ...
                $(this).find('td').each(function () {
                    if(title[loop] == "commune" || title[loop] == "refSituationFamiliale" || title[loop]=="refTaux" || title[loop]=="refMoyenPaiement"){
                        xml +="<"+title[loop]+"><code>"+clean_text($(this).text()) +"</code></"+title[loop]+">";
